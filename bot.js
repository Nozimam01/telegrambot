...
// ================= UNIVERSAL YUKLASH TIZIMI (ALL PLATFORMS FIX) =================
async function downloadAndSend(ctx, targetUrl, isAudio = false, customTitle = "", customPerformer = "") {
  ...
  // 1-BOSQICH: YUKLASH UCHUN UNIVERSAL FAST API (INSTAGRAM, TIKTOK VA YOUTUBE UCHUN)
  try {
    if (waiting) await ctx.telegram.editMessageText(ctx.chat.id, waiting.message_id, null, "📥 Server tayyorlanmoqda va yuklanmoqda...").catch(() => {});
    
    const response = await axios.post("https://api.anbusem.me/api/downloader", { url: targetUrl }, { timeout: 35000 });
    let directUrl = null;
    ...
    if (response.data && response.data.success && response.data.data) {
      ...
      if (Array.isArray(mediaList)) {
        ...
        directUrl = mediaList.url || mediaList.downloadUrl;
      }
    }
    ...
    if (directUrl) {
      ...
      if (isAudio) {
        await ctx.replyWithAudio({ source: finalPath, filename: `Audio_${fileId}.mp3` }, { title: customTitle || "Audio Track", performer: customPerformer || "Media Downloader" });
      } else {
        await ctx.replyWithVideo({ source: finalPath }, { caption: `🎬 <b>${customTitle || "Yuklab olindi!"}</b>\n\n📥 @${ctx.botInfo.username} orqali yuklandi`, parse_mode: "HTML" });
      }
    ...
    console.log("Fast API ishlamadi, yt-dlp zaxirasiga o'tilmoqda:", apiErr.message);
  }

  // 2-BOSQICH: ZAXIRA UCHUN YT-DLP ENGINE
  ...
  if (!videoTitle && (targetUrl.includes("youtube.com") || targetUrl.includes("youtu.be"))) {
    try {
      const searchResults = await ytSearch({ query: targetUrl });
      if (searchResults && searchResults.videos && searchResults.videos.length > 0) {
        videoTitle = searchResults.videos[0].title.replace(/[<>:"/\\|?*]/g, "").trim();
        performerName = searchResults.videos[0].author?.name || "YouTube Player";
      }
    } catch (e) {}
  }
...
// MongoDB ulanishi va WEBHOOK
client.connect().then(async () => {
  console.log("🍃 MongoDB Client ulandi!");
  
  const webhookUrl = `${RENDER_URL}${SECRET_PATH}`;
  await bot.telegram.setWebhook(webhookUrl, { drop_pending_updates: true });
  console.log(`🔥 PERSISTENT ENGINE RUNNING VIA WEBHOOK: ${webhookUrl}`);
